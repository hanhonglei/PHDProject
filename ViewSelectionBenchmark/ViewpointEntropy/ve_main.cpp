...
void display(void)
{
    ...
    float viewpoint_entropy[VIEWPOINT_CANDIDATE_NUMBER];
    memset(viewpoint_entropy,0,VIEWPOINT_CANDIDATE_NUMBER*sizeof(float));
    ...
    for (unsigned int iv=0; iv<VIEWPOINT_CANDIDATE_NUMBER; iv++)
    {
        ...
        std::vector<float> pcsb;
        pcsb.resize(mesh->faces.size());
        ...
        for (unsigned int i=0; i<ncsb.size(); i++)
        {
            pcsb[i] = ncsb[i]/sum_ncsb;
        }

        // compute entropy of pcsb
        for (unsigned int i=0; i<pcsb.size(); i++)
        {
            if(pcsb[i] > 0)
                viewpoint_entropy[iv] += -pcsb[i]*log(pcsb[i])/log(2.0f);
        }

        std::cout<<viewpoint_entropy[iv]<<std::endl;
        //save_result(viewpoint_entropy[iv]);
    }
    
    // select the best one
    float mve = 0; // maximum viewpoint entropy
    unsigned int bve_no = 0; //best viewpoint no.
    for (unsigned int i=0; i<VIEWPOINT_CANDIDATE_NUMBER; i++)
    {
        if (viewpoint_entropy[i] > mve)
        {
            mve = viewpoint_entropy[i];
            bve_no = i;
        }
    }
    ...
    std::ofstream veresult("veresult",std::ios::out | std::ios::app);
    veresult<<tcost<<viewpoint_candidates[bve_no]<<std::endl;
    veresult.close();
...
void main(int argc, char **argv)
{
    ...
    if (argc != 3)
    {
        std::cout<<"ve.exe model-file model-no"<<std::endl;
        return;
    }
