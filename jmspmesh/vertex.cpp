...
//	return (os << "<" << vo._myVertex.x << ", " << vo._myVertex.y << ", " << vo._myVertex.z << ">");
	...
	set<int>& myverNeighbors = ((vertex&)vo).getVertNeighbors();
	set<int>& mytriNeighbors = ((vertex&)vo).getTriNeighbors();


	for (pos = myverNeighbors.begin(); pos != myverNeighbors.end(); ++pos)
	{
		os << " " << *pos;
	}
	os << " Tri Neighbors:";
	for (pos = mytriNeighbors.begin(); pos != mytriNeighbors.end(); ++pos)
	{
		os << " " << *pos;
	}
...
void 
vertex::calcQuadric(jmsMesh& m, bool bUseTriArea)
{
	for (int i = 0; i < 4; ++i) {
		for (int j = 0; j < 4; ++j) {
			_Q[i][j] = 0;
...
bool vertex::isBorder(jmsMesh& m)
{
...
void vertex::getAllBorderEdges(set<border> &borderSet, jmsMesh& m)
{
	// So go through the list of all neighboring vertices, and see how many
	// triangles this vertex has in common w/ each neighboring vertex.  Normally
	// there will be two triangles in common, but if there is only one, then this 
