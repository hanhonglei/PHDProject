...
// vertex.h
//
// jmsMesh vertex has both a position and a normal
//
// Jeff Somers
// Copyright (c) 2002
//
...
public:
	...
	vertex() : 
		...
		_index(-1), _QTriArea(0), _layer(-1),_bound(false),_center(false)
	{
		initQuadric();
	};
	...
	vertex(const vertex& v) : _myVertex(v._myVertex), _vertexNormal(v._vertexNormal),
							...
							_index(v._index), _QTriArea(v._QTriArea), _layer(v._layer),_bound(v._bound),_center(v._center),_blur(v._blur)
	{
		// copy quadric
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
...
//	operator>>(istream& is, vertex& vi);
	...
	void calcQuadric(jmsMesh& m, bool bUseTriArea); // calculate the 4x4 Quadric matrix
	...
	bool isBorder(jmsMesh& m); // is this vertex on the border 
	...
	void getAllBorderEdges(set<border> &borderSet, jmsMesh& m);

	// 设置和得到当前顶点的模型分割层次信息，如果处于边界的话，取较小的值（父节点） [1/3/2011 Han Honglei]
	void setLayer(short nLayer) { if (_layer != nLayer && _layer != -1) _bound = true;	else if (nLayer < _layer || _layer == -1) _layer = nLayer;};
	short getLayer() { return _layer;};
	bool isBound(){return _bound;};
	double _blur;		// 增加运动模糊距离的度量信息

	void setPos(vec3 pos){ _myVertex = pos;}
	int skeletonNode;			// 该顶点所对应的骨架点编号
	// 在得到骨架点的过程中判断是否为center [7/14/2011 Han Honglei]
	bool _center;
...
private:
	...
	double _cost; // cost of removing this vertex from Progressive jmsMesh
	...
	// 增加顶点的模型分割层次信息，叶子结点先简化 [1/3/2011 Han Honglei]
	short _layer;
	bool _bound;		// 这个顶点处于两个层次的边界上
	// Used for Garland & Heckbert's quadric edge collapse cost (used for mesh simplifications/progressive meshes)
	void initQuadric()
	{
		for (int i = 0; i < 4; ++i) {
