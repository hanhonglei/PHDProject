...
Vec3 transformLoc(const Matrix& left_op, const Vec3& right_op)
{ return left_op * right_op; }

Vec3 transformVec(const Matrix& left_op, const Vec3& right_op)
{
   Vec3 ret;

   ret.x = right_op.x * left_op.x[0][0] + right_op.y * left_op.x[0][1] + 
			right_op.z * left_op.x[0][2];
   ret.y = right_op.x * left_op.x[1][0] + right_op.y * left_op.x[1][1] +
			right_op.z * left_op.x[1][2];
   ret.z = right_op.x * left_op.x[2][0] + right_op.y * left_op.x[2][1] +
			right_op.z * left_op.x[2][2];
...
Matrix
rotate(const Vec3 & axis, float angle)
{
   Vec3 _axis = axis/*unitVector(axis)*/;
   _axis.normalize();
   Matrix ret;
   float x = _axis.x;
   float y = _axis.y;
...
Matrix 
viewMatrix(const Vec3 & eye, const Vec3 & gaze, const Vec3 & up)
{
   Matrix ret = identityMatrix();

   // create an orthoganal basis from parameters
   Vec3 w = -gaze;/*-(unitVector(gaze))*/;
   w.normalize();
   Vec3 u = w.unitcross(up, w);/*unitVector(cross(up, w));*/
   Vec3 v = w.cross(w, u);
...
Vec3 operator* (const Matrix & left_op, const Vec3 & right_op)
{
   Vec3 ret;
   float temp;

   ret.x = right_op.x * left_op.x[0][0] + right_op.y * left_op.x[0][1] + 
			  right_op.z * left_op.x[0][2] +               left_op.x[0][3];
   ret.y = right_op.x * left_op.x[1][0] + right_op.y * left_op.x[1][1] +
			  right_op.z * left_op.x[1][2] +               left_op.x[1][3];
   ret.z = right_op.x * left_op.x[2][0] + right_op.y * left_op.x[2][1] +
			  right_op.z * left_op.x[2][2] +               left_op.x[2][3];
   temp   = right_op.x * left_op.x[3][0] + right_op.y * left_op.x[3][1] +
			  right_op.z * left_op.x[3][2] +               left_op.x[3][3];
