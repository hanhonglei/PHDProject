...
public:
	unsigned int nf;			// 用来标记当前Q来源于几个face，用于求平均Q [5/22/2012 Han]
   MxQuadric3() { clear(); nf = 1;}
    MxQuadric3(double a, double b, double c, double d, double area=1.0)
	{ init(a, b, c, d, area); nf = 1; }
    MxQuadric3(const float *n, double d, double area=1.0)
	{ init(n[X], n[Y], n[Z], d, area); nf = 1;}
    ...
    void clear(double val=0.0) { a2=ab=ac=ad=b2=bc=bd=c2=cd=d2=r=val; nf = 0;}
