...
MxStdModel *MxSMFReader::read_off(istream& in, MxStdModel *m)
{
    if( !m )
		m = new MxStdModel(8, 16);
	begin(0, NULL, *m);
	int vTotal = 0, fTotal = 0, eTotal = 0, vn = 0, fn = 0, en = 0;
	bool bGetVF = false;
	char line_tmp[SMF_MAXLINE];
	while( !in.eof() )
	{
		in >> ws;
		if( in.peek() == '#' )
		...
		{
			if (strcmp(line_buffer, "off") == 0 || strcmp(line_buffer, "OFF") == 0 )
				continue;
			if (!bGetVF)
			{
				bGetVF = true;
				sscanf (line_buffer,"%d %d %d",&vTotal,&fTotal, &eTotal);
				continue;
			}
			else
			{
				...
				if (vn < vTotal)	// vert
				{
					sprintf(line_tmp, "v %s", line_buffer);
					strcpy(line_buffer,line_tmp);
					vn++;
				}
				else				// face	smf顶点编号从1开始
				{
					int n3, nn1, nn2, nn3;
					sscanf (line_buffer,"%d %d %d %d",&n3,&nn1, &nn2, &nn3);
					sprintf(line_buffer, "f %d %d %d", nn1+1, nn2+1, nn3+1);
					//strcpy(line_buffer,line_tmp);
					//line_buffer[0] = 'f';
					fn++;
				}
...
void MxSMFWriter::write(ostream& out, MxStdModel& m)
{
    ...
    //out << "begin" << endl;
	// get the valid face num [11/24/2011 Han]
	int nFaceValid = 0;
	...
	out << "ply\nformat ascii 1.0\nelement vertex "<< m.vert_count() << "\nproperty float32 x\nproperty float32 y\nproperty float32 z\nelement face "<< nFaceValid << "\nproperty list uint8 int32 vertex_index\nend_header\n";
	...
	if( m.face_is_valid(f) )
	{
		...
		for (int i = 0; i < 3; i ++)
		{
			if (!m.vertex_is_valid(m.face(f)[i]))
			{
				int test = 1;
			}
		}
