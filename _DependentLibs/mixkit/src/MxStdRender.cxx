...
static inline void emit_vertex(MxStdModel& m, MxVertexID v)
{
    if( m.normal_binding()==MX_PERVERTEX ) 
		glN(m.normal(v));
	// 直接使用curve的颜色 [4/20/2012 Han]
	//if (m.v_infor_colors.length() > 0)	
	//	glColor3dv(m.v_infor_colors[v]);
	unsigned idx = m.vertex(v).colorIndex;
	idx = idx < 0?0:idx;
	idx = idx > 255?255:idx;
	glColor3dv(&m.LUT_CourbureClust[3*idx]);
