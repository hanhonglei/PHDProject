...
public:
    MxBlockModel(int nvert, int nface) : vertices(nvert), faces(nface)
	{
	    ...
	    tex = NULL;
	    tex_name = NULL;
		fMotionAngle = 0.0f;	fWeightPower = 1.0f;
	}
	...
	bool GetBoundSize(float size[3]);

	// 添加对逐顶点简化权重的支持 [3/30/2012 Han]
	// 保存顶点的motion blur数据 [3/29/2012 Han]
	std::vector<float> pVertexMBlur;
	float	fMotionAngle;
	float	fWeightPower;
	// 载入顶点运动模糊数据
	bool LoadMBlur(FILE* inFile);
